...
void test_setup_simple_populate()
{
	...
	strncpy(record.value, "col 9", sizeof record.value);
	status = storage_set(INTTABLE, KEY2, &record, test_conn);
	strncpy(record.value, "col 1", sizeof record.value);
...
START_TEST (test_query_int)
{
	// Do a query.  Expect no matches.
	int foundkeys = storage_query(INTTABLE, "col < 9, col > 5", test_keys, MAX_RECORDS_PER_TABLE, test_conn);
	...
	fail_unless(strcmp(test_keys[0], KEY1) == 0 , "Result was incorrect.\n");
}
END_TEST


START_TEST (test_query_str)
{
...
int main(int argc, char *argv[])
{
	...
	//Query for existent integers
	tcase_add_test(tc, test_query_int);
	//Query for an existent string
	tcase_add_test(tc, test_query_str);
