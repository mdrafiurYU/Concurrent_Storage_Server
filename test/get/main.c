...
#define EXTREMETABLES_CONF     "conf-extremetables.conf"        // Server configuration file with extreme tables.
...
#define EXTTABLE	"exttbl"	// The first simple table.
...
void test_setup_extreme_populate()
{
	test_conn = init_start_connect(EXTREMETABLES_CONF, "extremeempty.serverout", NULL);
	...
	strncpy(record.value, "col1 1,col2 1A,col3 1,col4 1A,col5 1,col6 1A,col7 1,col8 1A,col9 1,col10 1A", sizeof record.value);
	status = storage_set(EXTTABLE, KEY1, &record, test_conn);
...
void test_setup_simple_populate()
{
	...
	strncpy(record.value, "col ghi", sizeof record.value);
	status = storage_set(STRTABLE, KEY2, &record, test_conn);
	strncpy(record.value, "col abc def ghi", sizeof record.value);
...
START_TEST (test_getinvalid_badconn)
{
	test_teardown();
	struct storage_record record;
	int status = storage_get(THREECOLSTABLE, KEY1, &record, test_conn);
	fail_unless(status == -1, "storage_get with bad conn should fail.");
}
END_TEST


START_TEST (test_getexistentvalue)
{
	struct storage_record record;
	int status = storage_get(THREECOLSTABLE, KEY1, &record, test_conn);
	fail_unless(status == 0, "storage_get with existent key should pass.");
	fail_unless(strcmp(record.value, "col1 -2,col2 -2,col3 abc") == 0, "Got wrong value.");
}
END_TEST
...
int main(int argc, char *argv[])
{
	...
	// Get tests with bad auth
	tc = tcase_create("getbadconn");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_complex_populate, test_teardown);
	tcase_add_test(tc, test_getinvalid_badconn);
	suite_add_tcase(s, tc);


	// Get test with existent record
	tc = tcase_create("getexistentrecord");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_complex_populate, test_teardown);
	tcase_add_test(tc, test_getexistentvalue);
	suite_add_tcase(s, tc);
