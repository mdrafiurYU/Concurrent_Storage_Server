...
START_TEST (test_setcomplex_updatethreecols) {
	...
	status = storage_get(THREECOLSTABLE, KEY1, &record, test_conn);
	fail_unless(status == 0, "Error getting a value.");
	fields = sscanf(record.value, "col1 %d , col2 %d , col3 %[a-zA-Z0-9 ]", &intval, &floatval, strval);
...
/*
 * Transaction tests:
 *      get/set tx that passes.
 *      get/set tx that fails.
 *      get/set tx, force set.
 *
 */

START_TEST (test_transaction_pass1)
{
        struct storage_record record;
        memset(&record, 0, sizeof record);
        int fields = 0;
        ...
        // Get a record.
        strncpy(record.value, "", sizeof record.value);
        status = storage_get(INTTABLE, KEY2, &record, test_conn);
        fail_unless(status == 0, "Error getting a value.");
        fields = sscanf(record.value, "col %d", &intval);
        fail_unless(fields == 1 && intval == 2, "Got wrong value.");

        // Update the record (as part of the tx).
        strncpy(record.value, "col 8", sizeof record.value);
        status = storage_set(INTTABLE, KEY2, &record, test_conn);
        fail_unless(status == 0, "Error setting a key/value pair.");
        ...
        fail_unless(status == 0, "Error getting a value.");
        fields = sscanf(record.value, "col %d", &intval);
        fail_unless(fields == 1 && intval == 8, "Got wrong value.");

}
END_TEST

START_TEST (test_transaction_fail1)
{
        ...
        // Get a record (part of tx1).
        strncpy(record1.value, "", sizeof record1.value);
        status = storage_get(INTTABLE, KEY2, &record1, test_conn);
        fail_unless(status == 0, "Error getting a value.");
        fields = sscanf(record1.value, "col %d", &intval);
        fail_unless(fields == 1 && intval == 2, "Got wrong value.");

        // Update the record (forced).
        memset(&record2, 0, sizeof record2);
        strncpy(record2.value, "col 8", sizeof record2.value);
        status = storage_set(INTTABLE, KEY2, &record2, test_conn);
        fail_unless(status == 0, "Error setting a key/value pair.");

        // Update the record (part of tx1).  Should fail.
        strncpy(record1.value, "col 88", sizeof record1.value);
        status = storage_set(INTTABLE, KEY2, &record1, test_conn);
        fail_unless(status == -1, "storage_set for aborted transaction should fail.");
        fail_unless(errno == ERR_TRANSACTION_ABORT, "storage_set for aborted transaction not setting errno properly.");

        // Make sure the record has right value.
        strncpy(record2.value, "", sizeof record2.value);
        status = storage_get(INTTABLE, KEY2, &record2, test_conn);
        fail_unless(status == 0, "Error getting a value.");
        fields = sscanf(record2.value, "col %d", &intval);
        fail_unless(fields == 1 && intval == 8, "Got wrong value.");
...
/*
 * Concurrency tests:
 *      get with two connections.
 *      set/get with two connections.
 *      query with two connections.
 *
 */
START_TEST (test_concurrency_get)
{
        struct storage_record record;
        memset(&record, 0, sizeof record);
        int status = 0;
        ...
        void *conn1 = test_conn;
        void *conn2 = NULL;

        // Make a second connection to the server.
        conn2 = storage_connect(SERVERHOST, server_port);
        fail_unless(conn2 != NULL, "Couldn't connect to server.");

	// Authenticate with the server.
	status = storage_auth(SERVERUSERNAME,
				  SERVERPASSWORD,
				  conn2);
	fail_unless(status == 0, "Authentication failed.");
        ...
        // Do a get on conn1.
        strncpy(record.value, "", sizeof record.value);
        status = storage_get(INTTABLE, KEY1, &record, conn1);
	printf("status: %d\n",status);
        fail_unless(status == 0, "Error getting a value.");
        fields = sscanf(record.value, "col %d", &intval);
        fail_unless(fields == 1 && intval == -2, "Got wrong value.");
        ...
        // Do a get on conn2.
        strncpy(record.value, "", sizeof record.value);
        status = storage_get(FLOATTABLE, KEY1, &record, conn2);
	printf("status: %d\n",status);
        fail_unless(status == 0, "Error getting a value.");
        fields = sscanf(record.value, "col %d", &floatval);
        fail_unless(fields == 1 && floatval == -2, "Got wrong value.");

        // Do a get on conn1.
        strncpy(record.value, "", sizeof record.value);
        status = storage_get(STRTABLE, KEY1, &record, conn1);
	printf("status: %d\n",status);
        fail_unless(status == 0, "Error getting a value.");
        fields = sscanf(record.value, "col %[a-zA-Z0-9 ]", strval);
        fail_unless(fields == 1 && strcmp(trimtrailingspc(strval), "abc") == 0, "Got wrong value.");

        // Disconnect from the server.
        storage_disconnect(conn2);
}
END_TEST

START_TEST (test_concurrency_setget)
{
        struct storage_record record;
        memset(&record, 0, sizeof record);
        int status = 0;
	...
	fail_unless(status == 0, "Authentication failed.");
        ...
        memset(&record, 0, sizeof record);
        strncpy(record.value, "col 8", sizeof record.value);
        status = storage_set(INTTABLE, KEY2, &record, conn1);
        fail_unless(status == 0, "Error setting a key/value pair.");

        // Do a set on conn2.
        memset(&record, 0, sizeof record);
        strncpy(record.value, "col 88", sizeof record.value);
        status = storage_set(INTTABLE, KEY2, &record, conn2);
        fail_unless(status == 0, "Error setting a key/value pair.");

        // Do a get on conn1.
        strncpy(record.value, "", sizeof record.value);
        status = storage_get(INTTABLE, KEY2, &record, conn1);
        fail_unless(status == 0, "Error getting a value.");
        fields = sscanf(record.value, "col %d", &intval);
        fail_unless(fields == 1 && intval == 88, "Got wrong value.");

        // Do a get on conn2.
        strncpy(record.value, "", sizeof record.value);
        status = storage_get(INTTABLE, KEY2, &record, conn2);
        fail_unless(status == 0, "Error getting a value.");
        fields = sscanf(record.value, "col %d", &intval);
        fail_unless(fields == 1 && intval == 88, "Got wrong value.");
...
START_TEST (test_concurrency_txpass)
{
        struct storage_record record;
        memset(&record, 0, sizeof record);
        int fields = 0;
	...
	fail_unless(status == 0, "Authentication failed.");
        ...
        // Update the record using conn2 (as part of the tx).
        strncpy(record.value, "col 8", sizeof record.value);
        status = storage_set(INTTABLE, KEY2, &record, conn2);
        fail_unless(status == 0, "Error setting a key/value pair.");

        // Make sure the record was updated using conn1.
        strncpy(record.value, "", sizeof record.value);
        status = storage_get(INTTABLE, KEY2, &record, conn1);
        fail_unless(status == 0, "Error getting a value.");
        fields = sscanf(record.value, "col %d", &intval);
...
/**
 * @brief This runs the marking tests for Assignment 4.
 */
int main(int argc, char *argv[])
{
	...
	Suite *s = suite_create("a4-partial");
	TCase *tc;
...
//	tcase_add_test(tc, test_set_updatefloat);
	...
	suite_add_tcase(s, tc);
	 ...
	 // Transaction tests
        tc = tcase_create("transaction");
        tcase_set_timeout(tc, TESTTIMEOUT);
        tcase_add_checked_fixture(tc, test_setup_simple_populate, test_teardown);
        tcase_add_test(tc, test_transaction_pass1);
        tcase_add_test(tc, test_transaction_fail1);
        suite_add_tcase(s, tc);
        ...
        tcase_set_timeout(tc, TESTTIMEOUT);
        tcase_add_checked_fixture(tc, test_setup_simple_populate, test_teardown);
        tcase_add_test(tc, test_concurrency_get);
        tcase_add_test(tc, test_concurrency_setget);
        tcase_add_test(tc, test_concurrency_txpass);
        suite_add_tcase(s, tc);
