...
START_TEST (test_change_values)
{
	struct storage_record record;
	strncpy(record.value, "col -2" , sizeof record.value);	

	// Delete a key/value pair.
	int status = storage_set(INTTABLE, KEY1, record.value, test_conn);
	fail_unless(status == 0, "status should be 0.");

	// Try to get the deleted value.
	strncpy(record.value, "", sizeof record.value);
	status = storage_get(INTTABLE, KEY1, &record, test_conn);
	fail_unless(status == 0, "storage_get for edited value should pass.");
	fail_unless(strcmp(record.value, "col -2") == 0, "value of record should be 2");
}
END_TEST

//Test by disconnection, and setting.  Server should not allow this.
START_TEST (test_set_with_disconnection)
{
	//Disconnect
	test_teardown();
	...
	strncpy(record.value, "col -2" , sizeof record.value);	
	int status = storage_set(INTTABLE, KEY1, record.value, test_conn);
	fail_unless(status == -1, "status should be -1.");
}
END_TEST
...
int main(int argc, char *argv[])
{
	...
	tc = tcase_create("Changing existent record's value");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_simple_populate, test_teardown);
	tcase_add_test(tc, test_change_values);
	suite_add_tcase(s, tc);

	// Set tests on simple tables
	//Test will disconnect, and try to change
	tc = tcase_create("Disconnection, then trying to set record value");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_simple_populate, test_teardown);
	tcase_add_test(tc, test_set_with_disconnection);
	suite_add_tcase(s, tc);
